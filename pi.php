...
if (!$crypto) {
    echo "Cryptomonnaie PI non trouvée.";
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['buy_amount'])) {
    ...
    if ($price <= 0) {
        $errors[] = "Prix introuvable pour PI.";
    } elseif ($amount <= 0) {
        ...
        if ($balance >= $cost) {
            ...
            $success = "Achat de $amount PI effectué avec succès !";
        } else {
    ...
    if ($price <= 0) {
        ...
        if ($res && $row = $res->fetch_assoc()) {
            if ($row['amount'] >= $amount) {
                ...
                $errors[] = "Quantité insuffisante (vous possédez {$row['amount']} PI).";
            }
        } else {
            $errors[] = "Vous ne possédez aucun PI.";
        }
    }
}
...
<div class="main">
    <div class="info-section">
        ...
        <div><h2>Graphique</h2><img src="<?php echo htmlspecialchars($crypto['graph_url']); ?>" alt="Graph PI"></div>
    </div>
