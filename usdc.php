...
if (!$crypto) {
    echo "Cryptomonnaie USDC non trouvée.";
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['buy_amount'])) {
    ...
    if ($price <= 0) {
        $errors[] = "Prix introuvable pour USDC.";
    } elseif ($amount <= 0) {
        ...
        if ($balance >= $cost) {
            ...
            $success = "Achat de $amount USDC effectué avec succès !";
        } else {
    ...
    if ($price <= 0) {
        ...
        if ($res && $row = $res->fetch_assoc()) {
            if ($row['amount'] >= $amount) {
                ...
                $errors[] = "Quantité insuffisante (vous possédez {$row['amount']} USDC).";
            }
        } else {
            $errors[] = "Vous ne possédez aucun USDC.";
        }
    }
}
...
<div class="main">
    <div class="info-section">
        ...
        <div><h2>Graphique</h2><img src="<?php echo htmlspecialchars($crypto['graph_url']); ?>" alt="Graph USDC"></div>
    </div>
