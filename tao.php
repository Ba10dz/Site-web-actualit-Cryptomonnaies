...
if (!$crypto) {
    echo "Cryptomonnaie TAO non trouvée.";
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['buy_amount'])) {
    ...
    if ($price <= 0) {
        $errors[] = "Prix introuvable pour TAO.";
    } elseif ($amount <= 0) {
        ...
        if ($balance >= $cost) {
            ...
            $success = "Achat de $amount TAO effectué avec succès !";
        } else {
    ...
    if ($price <= 0) {
        ...
        if ($res && $row = $res->fetch_assoc()) {
            if ($row['amount'] >= $amount) {
                ...
                $errors[] = "Quantité insuffisante (vous possédez {$row['amount']} TAO).";
            }
        } else {
            $errors[] = "Vous ne possédez aucun TAO.";
        }
    }
}
...
<div class="main">
    <div class="info-section">
        ...
        <div><h2>Graphique</h2><img src="<?php echo htmlspecialchars($crypto['graph_url']); ?>" alt="Graph TAO"></div>
    </div>
